...
class PatchFormer(nn.Module):#结果不错，可行
    def __init__(self,patch_len,seq_len,pred_len,
        ...
        ##intra_patch_attention(SE Attention)
        self.intra_embeddings = nn.Parameter(torch.rand(self.patch_nums, 1, 1, enc_in, 16),
                                                requires_grad=True)
        self.embeddings_generator = nn.ModuleList([nn.Sequential(*[nn.Linear(16, d_model)]) for _ in range(self.patch_nums)])
        self.intra_d_model = d_model
        self.intra_patch_attention = FeatureAttentionSE4D(self.patch_size+1)
        self.intra_Linear = nn.Linear(self.patch_nums, self.patch_nums*self.patch_size)

        ##inter_patch_attention(SE Attention)
        self.stride = self.patch_size
        self.inter_patch_attention = FeatureAttentionSE4D(self.patch_nums)

        # ##temporal_feature_fusion(SE Attention)
        self.fusion = FeatureAttentionSE4D(2*seq_len)
        ...
        self.head = nn.Linear(2*seq_len*d_model,pred_len)
    ...
    def forward(self,x):
        ...
        ####Intra-patch Attention(SE Attention)##### 
        for i in range(self.patch_nums):
            ...
            t = torch.cat([intra_emb, t], dim=1) # [B,1+P,M,D]
            t = self.intra_patch_attention(t)
            out = torch.reshape(t[:,0,:,:],(t.shape[0],1,t.shape[-2],t.shape[-1])) # [B,1,M,D]
        ...
        ####Inter-patch Attention(SE Attention)#####
        ...
        # ####temporal FeatureFusion(SE Attention)#####
        out = torch.concat([intra_out_concat, inter_out],dim=1)#2*[B,L,M,D]->[B,2*L,M,D]
        out = self.fusion(out)+out#[B,2*L,M,D]->[B,2*L,M,D]
        out = torch.sum(out, dim=-2)#[B,2*L,M,D]->[B,2*L,D]
        intra_out = out[:,:L,:]
        inter_out = out[:,L:,:]
        out = torch.concat([intra_out,inter_out],dim=2)
...
class PatchFormerNetModel(BaseModel):
    def __init__(self,patch_len,seq_len,pred_len,enc_in, 
                 d_model=16,factor=3, dropout=0.1, output_attention=False, n_heads=8, activation='gelu', e_layers=2,**kwargs):
